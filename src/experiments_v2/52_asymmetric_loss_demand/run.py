"""
Experiment 52: Asymmetric loss on DEMAND target.

Same as exp 42 (alpha=0.6 penalizes under-prediction) but trained on Spros (demand).
Data: daily_sales_8m_demand.csv with demand lags.
Baseline: exp 03 Model B (MAE 2.9923, WMAPE 28.19% vs Spros).

Usage:
  .venv/Scripts/python.exe src/experiments_v2/52_asymmetric_loss_demand/run.py
"""
...
def main():
    print("=" * 60)
    print(f"  EXPERIMENT 52: Asymmetric loss (alpha={ALPHA}) (DEMAND target)")
    print("=" * 60)
    t_start = time.time()
    ...
    print(f"  Training with asymmetric loss (alpha={ALPHA}) on {DEMAND_TARGET}...")
    t_train = time.time()
    ...
    print_metrics("52_asymmetric_demand", y_test, y_pred, baseline_mae=BASELINE_MAE)
    ...
    metrics = {
        "experiment": "52_asymmetric_loss_demand",
