"""
Experiment 51: Log-transform target on DEMAND.

Same as exp 41 (log1p/expm1) but trained on Spros (demand).
Data: daily_sales_8m_demand.csv with demand lags.
Baseline: exp 03 Model B (MAE 2.9923, WMAPE 28.19% vs Spros).

Usage:
  .venv/Scripts/python.exe src/experiments_v2/51_log_target_demand/run.py
"""
...
def main():
    print("=" * 60)
    print("  EXPERIMENT 51: Log-transform target (DEMAND)")
    print("=" * 60)
    t_start = time.time()
    ...
    X_train, y_train_raw = train[available], train[DEMAND_TARGET]
    ...
    # Train on log target
    print(f"  Training on log1p({DEMAND_TARGET})...")
    t_train = time.time()
    ...
    print_metrics("51_log_target_demand", y_test, y_pred, baseline_mae=BASELINE_MAE)
    ...
    metrics = {
        "experiment": "51_log_target_demand",
