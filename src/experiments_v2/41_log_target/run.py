"""
Experiment 41: Log-transform target.

y = log1p(Продано), predict, then expm1(pred).
Compresses scale: difference between 150 and 200 = 0.28 instead of 50.
Reduces dominance of high demand in loss.

Risk: may worsen low demand (0 vs 1 gets amplified in log space).

Usage:
  .venv/Scripts/python.exe src/experiments_v2/41_log_target/run.py
"""
...
def main():
    print("=" * 60)
    print("  EXPERIMENT 41: Log-transform target")
    print("=" * 60)
    t_start = time.time()
    ...
    X_train, y_train_raw = train[available], train[TARGET]
    X_test, y_test = test[available], test[TARGET]

    # Log-transform target
    y_train_log = np.log1p(y_train_raw)
    print(f"  Target stats: raw mean={y_train_raw.mean():.2f}, log mean={y_train_log.mean():.2f}")

    # Train on log target
    print(f"  Training on log1p(target)...")
    t_train = time.time()
    model = train_lgbm(X_train, y_train_log)
    ...
    print(f"\n  === RESULTS ===")
    print_metrics("41_log_target", y_test, y_pred, baseline_mae=BASELINE_MAE)
    ...
    metrics = {
        "experiment": "41_log_target", "transform": "log1p/expm1",
