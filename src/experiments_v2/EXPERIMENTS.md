...
## Tier 3: Benchmarking and Routing

### 67. Bakery/SKU benchmark harness вњ…
Shared benchmark on the same bakery set and SKU contract.
- Default global model from `66_cluster_features`
- Local SKU model
- Two-week average fallback
- Comparison outputs per SKU and per model
- Purpose: find stable routing rules, not per-bakery specialization

### 68. Prophet benchmark вњ…
Prophet-only benchmark on the same SKU subset, filtered by `global_best_r2`.
- Prophet base
- Prophet holidays
- Prophet calendar
- Prophet calendar + holidays
- Prophet safe regressors
- Purpose: isolate where Prophet is genuinely better than the global tabular model
...
**Phase 7 — Benchmarking and routing:**
67 (global / SKU / fallback benchmark) → 68 (Prophet benchmark)
