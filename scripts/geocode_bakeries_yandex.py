"""Geocode bakeries from bakery_geo_master.csv via Yandex Geocoder API.

Examples:
  python scripts/geocode_bakeries_yandex.py --api-key YOUR_KEY
  python scripts/geocode_bakeries_yandex.py --limit 20
"""
...
from src.geocoding_yandex import geocode_bakery_row, get_yandex_api_key  # noqa: E402
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Geocode bakeries from bakery_geo_master.csv "
            "via Yandex Geocoder API"
        )
    )
    parser.add_argument(
        "--master-csv",
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Yandex Geocoder API key. If omitted, YANDEX_GEOCODER_API_KEY is used.",
    )
    parser.add_argument(
        "--limit",
        type=int,
    ...
    api_key = get_yandex_api_key(args.api_key)
