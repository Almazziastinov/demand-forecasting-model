...
def _ration(
    ...
    baker_min_needed: dict[str, float],
    tray_slots: int,
    baker_minutes: float,
) -> dict[str, float]:
    ...
    for group_members in _group_and_rank(candidates):
        ...
        group_baker_min = sum(baker_min_needed[s.product_id] for s in group_members)
        if group_trays <= remaining_trays and group_baker_min <= remaining_baker_min:
            for sku in group_members:
                scheduled[sku.product_id] = trays_needed[sku.product_id] * sku.kratnost
...
def allocate_greedy(
    ...
    for w, window in enumerate(windows):
        ...
        if candidates:
            ...
            baker_min_needed: dict[str, float] = {}
            for sku in candidates:
                trays = math.ceil(target[sku.product_id] / sku.kratnost)
                trays_needed[sku.product_id] = trays
                molding_min = resolve_molding_minutes(sku.category_name, molding_minutes_map)
                baker_min_needed[sku.product_id] = trays * sku.kratnost * molding_min

            cap = window_capacity(window, capacity)
            total_trays = sum(trays_needed.values())
            total_baker_min = sum(baker_min_needed.values())

            if total_trays <= cap.tray_slots and total_baker_min <= cap.baker_minutes:
                scheduled = {
                    sku.product_id: trays_needed[sku.product_id] * sku.kratnost
                    for sku in candidates
                }
            else:
                scheduled = _ration(
                    candidates, trays_needed, baker_min_needed, cap.tray_slots, cap.baker_minutes
                )
