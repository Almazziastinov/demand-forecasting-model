...
def allocate_milp_detailed(
    ...
    mandatory_first_window_ids: frozenset[str] = frozenset(),
    ...
    `mandatory_first_window_ids` — product_ids that must be baked in window 0
    (the earliest window, e.g. 4:00-7:00). A lower-bound of 1 tray is set for
    trays[i, 0] for each such SKU.

    `max_active_windows` — if set, at most this many distinct baking windows
    may carry non-zero production across all SKUs. Implemented via binary
    y[w] ∈ {0,1} "window active" variables with big-M linking constraints and
    a cardinality upper bound. The first window is implicitly forced active
    whenever `mandatory_first_window_ids` is non-empty.
    """
    ...
    # y[w] (window-active binary): n_win — only when max_active_windows is set.
    has_window_limit = max_active_windows is not None
    n_y = n_win if has_window_limit else 0
    n_vars = 2 * n_regular + n_sku + n_y
    ...
    # Variable bounds: default lb=0 for all; mandatory-first-window SKUs get
    # lb=1 on trays[i,0] to force at least one tray in the first window.
    lb_vars = np.zeros(n_vars)
    ub_vars = np.full(n_vars, np.inf)
    if mandatory_first_window_ids and n_win > 0:
        for i, sku in enumerate(skus):
            if sku.product_id in mandatory_first_window_ids:
                lb_vars[trays_idx(i, 0)] = 1.0

    # Window-count limit: binary y[w] ∈ {0,1}, trays[i,w] <= M_i * y[w],
    # sum(y[w]) <= max_active_windows.
    if has_window_limit:
        ub_vars[2 * n_regular + n_sku:] = 1.0  # y[w] in [0, 1]

        # Per-SKU big-M: tightest safe upper bound on trays for that SKU.
        bigM = np.array(
            [
                math.ceil(
                    (sum(skus[i].hourly_qty.values()) + mandatory_extra[i])
                    / max(kratnost[i], 1)
                )
                + 2
                for i in range(n_sku)
        ...
        # trays[i, w] - M_i * y[w] <= 0  for all i, w
        n_bigm = n_sku * n_win
        a_bigm = np.zeros((n_bigm, n_vars))
        for i in range(n_sku):
            for w in range(n_win):
                r = i * n_win + w
                a_bigm[r, trays_idx(i, w)] = 1.0
                a_bigm[r, y_idx(w)] = -bigM[i]
        constraints.append(LinearConstraint(a_bigm, np.full(n_bigm, -np.inf), np.zeros(n_bigm)))

        # sum(y[w]) <= max_active_windows
        a_ysum = np.zeros((1, n_vars))
        for w in range(n_win):
            a_ysum[0, y_idx(w)] = 1.0
        constraints.append(LinearConstraint(a_ysum, np.array([-np.inf]), np.array([float(max_active_windows)])))

    integrality = np.zeros(n_vars)
    integrality[:n_regular] = 1  # trays integer; both shortfall families stay continuous
    if has_window_limit:
        integrality[2 * n_regular + n_sku:] = 1  # y[w] binary
...
def allocate_milp(
    ...
    regular, defrost_out, two_day_out, _shortfall_by_sku, _defrost_shortfall_by_sku = allocate_milp_detailed(
        ...
        mandatory_first_window_ids=mandatory_first_window_ids,
